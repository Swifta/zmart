        ...
        <form action="" method="post" class="admin_form" enctype="multipart/form-data">
            <table>
                ...
                <tr>
                    <td></td>
                    <td></td>
                    <td><input type="submit" value="<?php echo $this->Lang['SUBMIT']; ?>" /><input type="button" value="<?php echo $this->Lang['RESET']; ?>" onclick="javascript:window.location='<?php echo PATH; ?>store-admin/couopn_code.html'"/></td>
                </tr>
            </table>
        </form>
...
<?php }?>
			...
			<th align="left" width="5%"><?php echo $this->Lang["SAVINGS"]; ?><?php echo '('.CURRENCY_SYMBOL.')';?></th>
		        <th align="left" width="10%"><?php echo $this->Lang["COUPON_CODE"]; ?></th>
			...
			<?php  if($u->type =="5"){ ?>
				...
				<?php	} ?>
	                ...
	                <td><?php $type = ($u->type == 5)?1:0; if($u->coupon_code_status == 1){  ?>
						...
						<a onclick="return close_deal('<?php echo $u->deal_id; ?>','<?php echo $u->coupon_code; ?>','close','<?php echo $u->trans_id; ?>','<?php echo $type; ?>');" class="close"></a>
