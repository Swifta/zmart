...
<form method="get" class="admin_form fl" action="">
		...
		<?php  if(($this->uri->last_segment() == 'manage-auction.html') || ($this->uri->last_segment()=='archive-auction.html') || ($this->uri->segment(3))){
       ...
       <label class="fl"> <b class="search_deal_title"><?php echo $this->Lang['SEARCH_AU_PROD']; ?> :</b></label>
        ...
        <div class="list_table1 fl clr date_range_part" id="date_display">
            ...
            <div class="date_range_right">
              <label>&nbsp;</label>        
              <span><?php echo $this->Lang['AUCTION_NAME']; ?>, <?php echo $this->Lang['STORE_NAME']; ?></span>
            </div> 
               </div>  
        ...
        <table class="list_table fl clr mt20">
			...
			<th align="left" width="20%"><div class="arrow"><a href="<?php echo $this->sort_url;?>param=name&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="<?php echo $this->Lang['SORT_DEALS']; ?>" ><?php echo $this->Lang["AUCTION_NAME"]; ?></a></div></th>
			...
			<th align="left" width="15%"><div class="arrow3"><a href="<?php echo $this->sort_url;?>param=price&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="<?php echo $this->Lang['SORT_BY_PRICE']; ?>" ><?php echo $this->Lang['AUC_PRICE']; ?> <?php echo '('.CURRENCY_SYMBOL.')';?></a></div></th>
			<?php /**<th align="left" width="15%"><div class="arrow4"><a href="<?php echo $this->sort_url;?>param=count&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="Sort By Count" >Bid Count</a></div></th> 
			**/?>
			<th align="left" width="15%"><div class="arrow5"><a href="<?php echo $this->sort_url;?>param=increment&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="<?php echo $this->Lang['SORT_BY_BIT_IN']; ?>" ><?php echo $this->Lang['BID_INCR']; ?> <?php echo '('.CURRENCY_SYMBOL.')';?></a></div></th>
			<th align="left" width="15%"><?php echo $this->Lang['AUC_IM']; ?></th> 
			<th align="left" width="15%"><?php echo $this->Lang["CREATED_DATE"]; ?></th>
			<?php if(($this->uri->last_segment() == 'manage-auction.html')||($this->uri->segment(2) == 'manage-auction.html')){ ?> <th align="left" ><?php echo $this->Lang['SEND_MAIL']; ?></th> 
			
			<?php if(ADMIN_PRIVILEGES_AUCTIONS_BLOCK || ADMIN_PRIVILEGES_AUCTIONS_EDIT){?>
			<th align="left" width="5%"><?php echo $this->Lang["ACTION"]; ?></th>
			<?php }?>
			
			<th align="left" width="10%"><?php echo $this->Lang['HOT_ACT']; ?></th>
			<?php } ?>
			
			<th align="left" width="5%"><?php echo 'Preview'; ?></th>
						
			<th align="left" width="10%"><?php echo $this->Lang["AUCT_DETA"]; ?></th>
                 </tr>
			...
			foreach($this->all_deal_list as $u){ ?>
                <tr>    
                        ...
                        <?php /**<td align="left"><?php echo $u->bid_count; ?></td> **/?>
                        <td align="left"><?php echo $u->bid_increment; ?></td>
         	                      
         	   <?php  if(file_exists(DOCROOT.'images/auction/1000_800/'.$u->deal_key.'_1'.'.png')) { ?>
         	   
                    <td align="left">
<img border="0" src= "<?php echo PATH.'resize.php';?>?src=<?php echo PATH.'images/auction/1000_800/'.$u->deal_key.'_1'.'.png';?>&w=80&h=80" alt="" />
<?php /* <img border="0" src= "<?php echo PATH.'images/auction/220_160/'.$u->deal_key.'_1'.'.png';?>" alt="" width="80" />*/ ?></td>
                     <?php } else { ?>
                 ...
                 <?php if(($this->uri->last_segment() == 'manage-auction.html')||($this->uri->segment(2) == 'manage-auction.html') && ($u->deal_status == 1)){ ?>
                 
                 <?php  if(($this->uri->last_segment() == 'manage-auction.html') && $u->deal_status == 1) { ?><td>
		<a title="Email to register users" href="<?php echo PATH.'admin-auction/send-auction-email/'.$u->deal_id.'/'.$u->deal_key.'.html';?>"><img src="<?php echo PATH;?>images/mail.png"></a></td> <?php } else{ echo "<td>---</td>"; }?>
			<?php if(ADMIN_PRIVILEGES_AUCTIONS_BLOCK || ADMIN_PRIVILEGES_AUCTIONS_EDIT){?>
		    <td align="left">
				<?php if(ADMIN_PRIVILEGES_AUCTIONS_EDIT){?>
			<?php if($u->auction_status == 2) { echo "---"; } else { ?>
			
                    <a href="<?php echo PATH.'admin/edit-auction/'.base64_encode($u->deal_id).'/'.$u->deal_key.'.html';?>" class="editicon" title="<?php echo $this->Lang['EDIT_DEAL']; ?>"></a>
                   <?php } }?>
                   <?php if(ADMIN_PRIVILEGES_AUCTIONS_BLOCK){?>
                   <?php if($u->deal_status == 1 && $u->store_status==1){ ?>
                    <a onclick="return blockunblockauction('<?php echo $u->deal_id; ?>','<?php echo $u->deal_key; ?>','block');" class="blockicon" title="<?php echo $this->Lang['BLOCK']; ?>"></a>
                    <?php } elseif($u->deal_status==1 && $u->store_status==0){  ?>
                    <a onclick="return blockunblockauction('<?php echo $u->deal_id; ?>','<?php echo $u->deal_key; ?>', 'unblock');" class="unblockicon" title="<?php echo $this->Lang['UNBLOCK']; ?>"></a>
	                <?php } else{  ?>
                    <a onclick="return blockunblockauction('<?php echo $u->deal_id; ?>','<?php echo $u->deal_key; ?>', 'unblock');" class="unblockicon" title="<?php echo $this->Lang['UNBLOCK']; ?>"></a>
	                <?php } }?>
                    ...
                    <input type="checkbox" <?php if($u->deal_feature == 1) { ?> checked <?php } ?> id="popular<?php echo $u->deal_id; ?>" value="<?php echo $u->deal_id; ?>" onclick="popular_auction(this.value,this)" />
                    <?php }else{echo "--";} ?>
                    </td>
                   
                    <?php } ?>
					<td align="left">
                    <a href="<?php echo PATH.$u->store_url_title.'/auction/'.$u->deal_key.'/'.$u->url_title.'/admin-preview.html';?>" class="previewicon" title="<?php echo $this->Lang['PREVIEW']; ?>" target="_blank"></a>
                    </td>
                    
		
                    <td align="left"><a href="<?php echo PATH.'admin/view-auction/'.$u->deal_key.'/'.$u->deal_id.'.html';?>"><?php echo $this->Lang["VIEW_DET"]; ?></a></td>
                </tr>
            <?php $i++; } ?>   
        </table> 
        </div>
