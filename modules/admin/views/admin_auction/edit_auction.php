    ...
    <div class="content_middle">
        ...
        <?php foreach($this->deal as $u){?>
            <table>
                ...
                <tr>
                    ...
                    <td>
                    	<select name="category" onchange="return change_category(this.value);">
							...
							<?php foreach($this->category_list as $main){ if($main->auction == 1 ) { ?>
               ...
               <tr id="category">
                    ...
                    <td>
                        ...
                        <select name="sub_category" onchange="return sec_change_auction_category(this.value);">
			<?php $cat_ids = explode(',',$u->sub_category_id); foreach($this->sub_category_list as $d){ if($u->category_id == $d->main_category_id) { ?>
                ...
                <tr id="sub_category">
                    ...
                    <td>
                        ...
                        <select name="sec_category" onchange="return third_change_auction_category(this.value);">
			...
			<?php $cat_ids = explode(',',$u->third_category_id); $i = 1; foreach($this->third_category_list as $d){ if($u->sec_category_id == $d->sub_category_id) { ?>
				...
				<?php } else{ if($i ==1) {?>
                    ...
                    <td>
                        <input type="hidden" name="deal_type" value="3" <?php if($u->deal_type == 3){ ?>checked="checked" <?php } ?>/> 
                 ...
                 <tr>
                    ...
                    <td><label>:</label></td>
                    <td>
                        <input type="text" name="product_price" maxlength="8" value="<?php echo $u->product_value; ?>" <?php if($u->bid_count !=0) { ?> readonly <?php } ?> />
		...
		<tr>
                ...
                <tr>
                    ...
                    <td><label>:</label></td>
                    <td>
                        <input type="text" name="bid_increment" maxlength="8" value="<?php echo $u->bid_increment; ?>" <?php if($u->bid_count !=0) { ?> readonly <?php } ?> />
                ...
                <td style="float:left;">
					<div class="merchant_store_img_edit">
							...
							<div class="uploadWrapper">
                                                        ...
                                                        <input type="file" name="image[]"  class="first" id="first<?php echo '_'.$i;?>" onchange="return validateFileExtension1(this,<?php echo $i;?>)"  style="width:24px;"  title="<?php echo $this->Lang['EDIT']; ?>"/>                                                        <?php if($i!=1){ ?>
                                                            <?php  if(file_exists(DOCROOT.'images/auction/1000_800/'.$u->deal_key.'_'.$i.'.png'))  { ?>

                                                                                                                       <a class="image_delete" href="<?php echo PATH; ?>delete-auction1-images.html?id=<?php echo $u->deal_id; ?>&img=<?php echo base64_encode($u->deal_key.'_'.$i);?>&deal_key=<?php echo $u->deal_key; ?>" onclick="return confirm('<?php echo $this->Lang["ARE_U_DEL"]; ?>')" title="<?php echo $this->Lang['DELETE']; ?>">
...
$(document).ready(function(){
    ...
    $("#upload<?php echo '_'.$i;?>").click(function() { $('#img_name').show();
        ...
        if (input.files && input.files[0]) {
            ...
            reader.readAsDataURL(input.files[0]);
                ...
                <tr>
                    <td></td>
                    <td></td>
                    <td><input type="submit" value="<?php echo $this->Lang['SUBMIT']; ?>" id="check2"/><input type="button" value="<?php echo $this->Lang['CANCEL']; ?>" onclick='window.location.href="<?php echo PATH?>admin/manage-auction.html"'/></td>
                </tr>
            </table><?php }?>
        </form>
