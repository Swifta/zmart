...
<div class="cont_container mt15 mt10">
    ...
    <form method="get" class="admin_form fl" action="">
        ...
        <table class="list_table fl clr mt20">
			...
			<th align="left" width="20%"><div class="arrow"><a href="<?php echo $this->sort_url;?>param=name&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="Sort By Deals Name" ><?php echo $this->Lang["DEALS_NAME"]; ?></a></div></th>
			...
			<th align="left" width="15%"><div class="arrow2"><a href="<?php echo $this->sort_url;?>param=store&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="Sort By Store Name" ><?php echo $this->Lang["STORE_NAME"]; ?></a></div></th>
			...
			<th align="left" width="15%"><div class="arrow4"><a href="<?php echo $this->sort_url;?>param=value&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="Sort By Deal Value" ><?php echo $this->Lang["DEALVALUE"]; ?><?php echo '('.CURRENCY_SYMBOL.')';?></a></div></th>
			 <?php /**<th align="left" width="15%"><div class="arrow5"><a href="<?php echo $this->sort_url;?>param=savings&sort=<?php if($this->input->get('sort')=='DESC'){ echo 'ASC'; }else{ echo 'DESC'; } ?>" title="Sort By Deal Savings" ><?php echo $this->Lang["SAVINGS"]; ?><?php echo '('.CURRENCY_SYMBOL.')';?></a></div></th> **/?>
			<th align="left" width="5%"> <?php echo $this->Lang['DEAL_IMG']; ?> </th>
			...
			<?php if(ADMIN_PRIVILEGES_DEALS_EDIT || ADMIN_PRIVILEGES_DEALS_BLOCK){?>			
			<th align="left" width="5%"><?php echo $this->Lang["ACTION"]; ?></th>
			<?php }?>
			
			<th align="left" width="10%"><?php echo $this->Lang['HOT_DEAL']; ?></th>
			<?php } ?>
			
			<th align="left" width="5%"><?php echo $this->Lang['PREVIEW']; ?></th>
			<?php if($this->more_action == $this->Lang['REQU']) { ?>
			<th align="left" width="12%"><?php echo $this->Lang['FORCE_CLOSE']; ?></th>
			<?php } ?>
			...
			<td align="left"><?php echo htmlspecialchars($u->city_name); ?></td>
                       ...
                       <?php /** <td align="left"><?php echo $u->deal_savings; ?></td> **/?>
                        ...
                        <td align="left">
<img border="0" src= "<?php echo PATH.'resize.php';?>?src=<?php echo PATH.'images/deals/1000_800/'.$u->deal_key.'_1'.'.png';?>&w=80&h=80" alt="" />
<?php /* <img border="0" src= "<?php echo PATH.'images/deals/220_160/'.$u->deal_key.'_1'.'.png';?>" alt="" width="80" /> */ ?></td>
                        <?php } else { ?>
                        ...
                        <?php if((($this->uri->last_segment() == 'manage-deals.html')||($this->uri->segment(2) == 'manage-deals.html')) ){ ?>
                       <?php  if(($this->uri->last_segment() == 'manage-deals.html') && $u->deal_status == 1) { ?> <td>
                        <a title="Email to register users" href="<?php echo PATH.'admin/send-deal-mail/'.$u->deal_id.'/'.$u->deal_key.'.html';?>"><img src="<?php echo PATH;?>images/mail.png"></a></td> <?php } else{ echo "<td>---</td>"; }?>
                        
                        <?php if(ADMIN_PRIVILEGES_DEALS_EDIT || ADMIN_PRIVILEGES_DEALS_BLOCK){?>
                        <td align="left">
							<?php if(ADMIN_PRIVILEGES_DEALS_EDIT){?>
							<a href="<?php echo PATH.'admin/edit-deal/'.base64_encode($u->deal_id).'/'.$u->deal_key.'.html';?>" class="editicon" title="<?php echo $this->Lang['EDIT_DEAL']; ?>"></a>
							<?php }?>
							<?php if(ADMIN_PRIVILEGES_DEALS_BLOCK){?>
                         <?php if($u->deal_status == 1 && $u->store_status==1){ ?>
                        <a onclick="return blockunblockdeal('<?php echo $u->deal_id; ?>','<?php echo $u->deal_key; ?>','block');" class="blockicon" title="<?php echo $this->Lang['BLOCK']; ?>"></a>
                        <?php } elseif($u->deal_status == 1 && $u->store_status==0){  ?>
                        <a onclick="return blockunblockdeal('<?php echo $u->deal_id; ?>','<?php echo $u->deal_key; ?>', 'unblock');" class="unblockicon" title="<?php echo $this->Lang['UNBLOCK']; ?>"></a>
                        <?php } else {  ?>
                        <a onclick="return blockunblockdeal('<?php echo $u->deal_id; ?>','<?php echo $u->deal_key; ?>', 'unblock');" class="unblockicon" title="<?php echo $this->Lang['UNBLOCK']; ?>"></a>
                        <?php } }?>
                        </td>  
                        ...
                        <input type="checkbox" <?php if($u->deal_feature == 1) { ?> checked <?php } ?> id="popular<?php echo $u->deal_id; ?>" value="<?php echo $u->deal_id; ?>" onclick="popular_deals(this.value,this)" />
                        <?php }else{echo "--";} ?>
                        </td>
                        <?php  } ?>
                        <td align="left">
                        <a href="<?php echo PATH.$u->store_url_title.'/deals/'.$u->deal_key.'/'.$u->url_title.'/admin-preview.html';?>" class="previewicon" title="<?php echo $this->Lang['PREVIEW']; ?>" target="_blank"></a>
                        </td>
                       
                        <?php if($this->more_action == $this->Lang['REQU']) { ?>
                        <td align="left">
                        <?php if(($u->purchase_count < $u->minimum_deals_limit) && ($u->enddate < time()) && ( $u->purchase_count > 0)){ ?>
                        <a onclick="return forceclose_deal('<?php echo $u->deal_id; ?>', '<?php echo $u->deal_key; ?>');" class="forceclose" title="<?php echo $this->Lang['CLOSE']; ?>"></a>
                        <?php } ?>
                        </td>
                        <?php } ?>
                        <td align="left"><a href="<?php echo PATH.'admin/view-deal/'.$u->deal_key.'/'.$u->deal_id.'.html';?>"><?php echo $this->Lang["VIEW_DET"]; ?></a></td>
                        </tr>
            <?php $i++; } ?>
        </table>
            </div>
