...
<div class="cont_container mt15 mt10" >
    ...
    <div class="content_middle">
	 ...
	 <?php if(count($this->shipping_list)>0){?>
			...
			<?php } ?>
				...
				<a class="fr frm_export" href="<?php echo PATH.$this->url.'?id='.$this->Lang['ALL2'].'&search='.$this->input->get('search'); ?>" title="<?php echo $this->Lang['EXP_ALL']; ?>"><?php echo $this->Lang['EXP_ALL']; ?></a>
	<?php  }	 ?>	
                <?php 
    ...
    <?php if(count($this->shipping_list)>0){?>
		...
		<?php /*<th align="left" ><?php echo $this->Lang["DELETE"]; ?></th> */ ?>
            ...
            <?php $i=0; $first_item = $this->pagination->current_first_item;
                ...
                <tr>    
						...
						<td align="left"><?php echo date('d-M-Y h:i:s A',$u->shipping_date); ?></td>	
     			 			...
     			 			<?php /* <td align="left"><?php echo $u->phone; ?></td>
			...
			<div class="popup_show1 popup_show_cod" id="lightboxdetails-panel<?php echo $u->shipping_id; ?>" style="display:none;"  >
...
if(coupon==''){
$('td#error-<?php echo $u->shipping_id; ?>').html("<?php echo $this->Lang['PLS_COUPON']; ?>");
return false;
}
else if(coupon != '' && amount != '' && trans_id != ''){
		
	$.post(path+'admin_products/validate_coupon?coupon='+coupon+'&amount='+amount+'&trans_id='+trans_id,{
				...
				if(data == 1){
					$('#form-<?php echo $u->shipping_id; ?>').submit();
						...
						$('td#error-<?php echo $u->shipping_id; ?>').html("<?php echo $this->Lang['PLS_COU_SHP']; ?>");
						return false;					
				}
		});
}
...
$('#details-cancel<?php echo $u->shipping_id; ?>').click(function(){ 
	...
	$('#lightbox-panel<?php echo $u->shipping_id; ?>').hide();
						...
						<?php } elseif($u->delivery_status==5) {
							...
							<select  onchange="return product_cod_delivery('<?php echo $u->email; ?>','<?php echo $u->firstname; ?>','<?php echo $u->shipping_id; ?>',this.value,'<?php echo $u->trans_id; ?>','<?php echo $u->product_id; ?>','<?php echo $u->merchant_id; ?>','0')">
