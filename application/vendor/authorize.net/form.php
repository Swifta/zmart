...
function calculate_referral2(ref_amt,price)
{
	...
	if((ref_amt_bal >= ref_amt ) && ((count*price)>=ref_amt)){
	 ...
	 {
	        alert("<?php echo $language['enter_applicable_referral_amount']; ?>"); 
		document.getElementById('ref_amt3').value = 0;
...
<div style="float:left;clear:both;width:600px; margin-left:5px;">
		<?php if($available_amt > 0) { ?>
	                <p style="color:#666;"><strong><?php echo $language["your_ref_bal"]; ?></strong> <?php echo CURRENCY; ?><span id="total_ref2"><?php echo $available_amt; ?></span><strong style="color:#666;" class="p5">.<?php echo $language["use_int"]; ?></strong></p>
		<?php } else { 
				$available_amt = 0;
		?>
		...
		<td style="font:bold 12px arial!important;color:#666!important;width:400px;" id="face"><label><?php echo $language['description']; ?></label></td>
		<td><label><?php echo $language['qty']; ?></label></td>
		<td></td>
		<td><label><?php echo $language['price']; ?></label></td>

		<?php if($available_amt > 0) { ?>
		<td><label><?php echo $language['referral_balance']; ?></label></td>
		<?php } ?>
		...
		<tr>
		<td style="font:normal 12px arial!important;color:#666!important;width:400px;float:left; clear:both;" id="face">
		<h5 style="font:normal 12px arial!important;color:#666!important;width:400px;float:left; clear:both;" id="face"><?php echo $language['payment_for'].' '.ucfirst($couponname); ?></h5>
		</td>
		<td>
		...
		<tr>
		<td colspan="7" style="border-bottom:1px dotted #C7C7C7;padding-bottom:15px;">
	...
	<tr>
		...
		<br />
		<span style="width:200px;font:arial 12px arial;clear:both;float:left;">(<?php echo $language['optional']; ?>)</span></td>
	...
	<tr>
		...
		<br />
		<span style="width:200px;font:arial 12px arial;clear:both;">(5 or 9 <?php echo $language['digits']; ?>)</span>
		</td>
	</tr>
	...
	<tr>
		...
		<td><input type="text" size="2" class="required" maxlength="2" name="countrycode" value="<?php //echo $getdetails->countrycode;?>" title="<?php echo $language['enter_your_country'];?>">
				<br />
		<span style="width:200px;font:arial 12px arial;clear:both;">(2 <?php echo $language['characters']; ?>)</span></td>
	</tr>
	<tr>
    <td>
	...
	<span class="submit fl clr ml15"><input class="bnone " type="Submit" title="<?php echo $language['pay_now']; ?>" value="<?php echo $language['pay_now']; ?>" id=""></span>
    </td>
    </tr>
    </table>
